const SupplyChainSimple= artifacts.require("./SupplyChainSimple.sol");
...
contract("SupplyChainSimple", accounts => {
  it("...should create first batch", async () => {
    
    // Get shpply chain contract at address
    const instance = await SupplyChainSimple.new(roles, steps, transition);

    // Add myself to role 'ROLE_MILK'
    await instance.addUserToRole(ROLE_MILK, { from: accounts[0] });
  ...
  it("...should create derived batch based on two prev batches", async () => {
    ...
    await instance.addUserToRole(ROLE_MILK,    { from: accounts[1] });
    await instance.addUserToRole(ROLE_BERRY,   { from: accounts[2] });
    await instance.addUserToRole(ROLE_YORGURT, { from: accounts[3] });
  ...
  it("...should not allow to execute step by a wrong role", async () => {
    ...
    await instance.addUserToRole(ROLE_BERRY, { from: accounts[0] });
  ...
  it("...should not allow to execute step with not enought precedent", async () => {
    ...
    // Add myself to role 'ROLE_BERRY'
    await instance.addUserToRole(ROLE_MILK ,   { from: accounts[1] });
    await instance.addUserToRole(ROLE_YORGURT, { from: accounts[2] });
